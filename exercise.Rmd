...
* `mx_heart`**: heart disease mortality per person-year**
* `mx_other`**: mortality due to all other causes, again per person-year**

# Build the period multiple decrement table

**Build a period multiple decrement table using this data. As part of this task, you will also construct a single decrement table. Aside from the first two age groups, where you will use the Coale and Demeny method, assume ${}_{n}a_x = n/2$.**

**Show your work in code. I will give partial credit for each step where you get the steps mostly right even if your answer ends up being numerically inaccurate. Each of the nine single decrement table steps is worth 0.25 points (subtotal: 2.25 points). Each of the remaining three steps in multiple decrement table construction is worth the same amount (subtotal: 0.75 points). I'll give you an additional 1 point for numerically accurate single decrement table results, and 2 points for numerically accurate multiple decrement table results. Total available: 6 points.**
...
## Briefly explain how the pattern you described above relates to the patterns in the ${}_{n}q_x^i$ values you calculated. (0.25 points)

## Among a hypothetical cohort who experiences this cause-specific mortality schedule, what proportion of those who will eventually die of heart disease will do so between the ages of 45 and 65? Show your work in $\LaTeX$. (0.25 points)

## Below, produce a table displaying the age-specific all-cause mortality rates. (0.25 points)
